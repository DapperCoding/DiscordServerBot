...
export class HostingHelper {
  ...
  public static getHostingPackages(client: Client) {
    new ApiRequestHandler(client).requestAPIWithType<any>(
      "GET",
      null,
      "hosting"
    );
  ...
  public static async createReactionHandler(
    client: Client,
    userId: string,
    channel: TextChannel,
    trueReaction?: string,
    falseReaction?: string
  ) {
    ...
    let tReact =
      trueReaction != null && trueReaction.length > 0 ? trueReaction : "👍";
    let fReact =
      falseReaction != null && falseReaction.length > 0 ? falseReaction : "👎";

    sent
      .awaitReactions(
        (reaction: MessageReaction, user: User) =>
          (reaction.emoji.name === tReact || reaction.emoji.name == fReact) &&
          user.id == userId,
        { max: 1 }
      )
      .then(collected => {
        ...
        if (user) {
          ...
          const embed = new RichEmbed()
            .setTitle(
              "Thanks for requesting contact about our hosting services"
            )
