...
export class ProductHelper {
    ...
    public static getProducts(client: Client): Promise<any[]> {
        ...
        return new ApiRequestHandler(client).requestAPIWithType<any[]>("GET", null, "product");
    }
    public static async createCustomBotReactionHandler(client: Client, userId: string, channel: TextChannel, message:Message|null) {
        let embed = new RichEmbed();

        embed.setTitle("DapperCoding Bot Service");

        embed.setDescription("Are you interested in buying a custom discord bot?\n\nIf you are, react with 👍 or react with 👎 if you are not");
        embed.setFooter(`If you want to view all of our current products, react with 🛒`)
                ...
                .then(collected => {
                    ...
                    if (userReaction.emoji.name === "👍") {
                        ...
                        const requestContactEmbed = new RichEmbed()
                            .setTitle("A user requested contact about the custom bot service")
                            .setDescription(`user ${user.username} (${user.id}) requested contact`);
                        const channel = sent?sent.guild.channels.find(x => x.name.toLowerCase() === "dapper-coding") as TextChannel : null;

                        if (channel) channel.send(requestContactEmbed);
                        if(sent) sent.channel.send("Thanks for requesting contact about our custom bot service, we'll inform you asap.")
                        if (sent) sent.delete();
                    } else if (userReaction.emoji.name === "🛒") {
                        const client = ClientHelper.getClient() as Client;
                        ProductHelper.createProductOverviewReactionHandler(client, userId, channel as TextChannel, sent);
                    } else {
                        if (sent) sent.delete();
                    }
    ...
    public static async createProductOverviewReactionHandler(client: Client, userId: string, channel: TextChannel, message:Message|null) {
        ProductHelper.getProducts(client).then(async products => {
            let embed = new RichEmbed();
            embed.setTitle("Product Catalog");
            embed.setFooter("React with one of the numbers to view the product\nClick on '🛒' to view the custom bot service\nOr cancel with '❌'");
            ...
            let mapped = products.map((v, index) => getEmojiForNumber(index)) as string[];
            let emojis = ['🛒', ...mapped, '❌'];
            ...
            sent.awaitReactions((reaction: MessageReaction, user: User) => {
                ...
                if (user) {
                    ...
                    const productId = productIndex >= 0 && productIndex < products.length ? products[productIndex].id : 0;
                    if (productId && productId > 0) {
                        ProductHelper.createProductInfoReactionHandler(productId, client, userId, channel, sent)
                    } else if (userReaction.emoji.name === "🛒") {
                        ProductHelper.createCustomBotReactionHandler(client, userId, channel, sent);
                    } else if (userReaction.emoji.name === "❌") {
                        if (sent) sent.delete();
                    } 
    ...
    public static async createProductInfoReactionHandler(id:number, client:Client,userId:string, channel:TextChannel, message:Message|null) {
      ...
      .then(async product => {
        ...
        sent.awaitReactions((reaction: MessageReaction, user: User) => ((reaction.emoji.name === "🛒" || reaction.emoji.name === "❌") && user.id == userId), { max: 1 }).then(collected => {
            const userReaction = collected.filter(x => x.users.filter(y => y.id == userId).size > 0).first();
            const user = userReaction ? userReaction.users.find(y => y.id == userId) : null;

           if (user) {
                if (userReaction.emoji.name === "🛒") {
                    ProductHelper.createProductOverviewReactionHandler(client, userId, channel, sent);
                } else {
                    if (sent) sent.delete();   
                }
           }
      ...
      .catch(err => {
          ...
          const teamChannel = channel.guild.channels.find(x=>x.name.toLowerCase() === "dapper-team") as TextChannel;
          
          teamChannel.send(`Error occured getting product information for id ${id} <@211115689807839232>`);
          console.error(err);
        });
    }
