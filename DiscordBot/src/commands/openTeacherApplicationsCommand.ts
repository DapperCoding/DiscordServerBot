...
export default class OpenApplicationsCommand extends BaseCommand {
  ...
  public getHelp(): IBotCommandHelp {
    return {
      caption: "?openTeacherApplications",
      description: "Sends a list of all teacher applications.",
      roles: ["recruiter"]
    };
  }
      ...
      .then(async applications => {
        ...
        if (applications.length == 0) {
          commandData.message.channel.send(
            "There are currently no open teacher applications"
          );
          return;
        }
              ...
              clickHandler: async data => {
                ...
                new ApiRequestHandler(commandData.client)
                  ...
                  .then(received => {
                    ...
                    try {
                      applicant.send(
                        `Congratulations! A member of the Dapper Recruiting team has requested an interview with you. Go to the channel: teacher-${data.application.id} to start the interview.`
                      );
                    } catch (e) {}
