...
export default class DebuggerCommand extends BaseCommand {
    ...
    public async process(commandData: CommandData): Promise<void> {
        ...
        let summoned = commandData.guild.member(commandData.message.mentions.users.first());
        if (!summoned) {
            commandData.message.delete();
            return;
        }

        let debuggingEmbed = this.createDebuggerEmbed(summoned, commandData.message);
