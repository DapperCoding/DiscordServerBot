...
export default class PrivateChannelCommand extends BaseCommand {
  ...
  public getHelp(): IBotCommandHelp {
    return {
      caption: "?privateChannel",
      description: "Tag the users you want a private channel with",
      roles: ["dapper coding", "moderator"]
    };
  }

  public async process(commandData: CommandData): Promise<void> {
    let category = commandData.message.guild.channels.find(
      x => x.name.toLowerCase() == "private-talks"
    );

    //Find the role 'Dapper Bot'
    var dapperRole = commandData.message.guild.roles.find(
      role => role.name === "Dapper Bot"
    );

    commandData.message.guild
      .createChannel(uuidv4(), "text", [
      ...
      .then(channel => {
        ...
        (channel as TextChannel).send(
          "I've created a private channel for you and " +
            commandData.message.mentions.members.size +
            " members."
        );
      });
  }
